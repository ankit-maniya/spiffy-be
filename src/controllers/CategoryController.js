...
const addCategory = async (req, res, next) => {
  try {
    ...
    bodyData.isApproved
      ...
      : 0;
    bodyData.categoryType
      ? (bodyData["categoryType"] = parseInt(bodyData.categoryType))
      : 0;
    ...
    if (isValidate.statuscode != 1) {
      if (
        ...
        await helper.removeCategoryFile(
          req.files.categoryBanner[0].filename,
          "TEMP"
        );
      }
...
const updateCategory = async (req, res, next) => {
  try {
    ...
    const isValidate = await CategorySchema.checkUpdateInputValidate(
      bodyData,
      restaurentId
    ...
    if (
      ...
      if (oldMenuData && oldMenuData.categoryBanner)
        //delete old file
        await helper.removeCategoryFile(
          oldMenuData.categoryBanner,
          "CATEGORY",
          restaurentId
        );
    }
...
const deleteCategory = async (req, res, next) => {
  try {
    ...
    const isValidate = await CategorySchema.checkDeleteInputValidate(
      req.params
    );
    if (isValidate.statuscode != 1) {
      throw { message: isValidate.message };
    ...
    // extrect the menuid
    const { categoryId } = req.params;
    await model.Category.findOneAndRemove({
      _id: categoryId,
      restaurentId,
    }).exec(async (delError, delResponse) => {
      if (delError) {
