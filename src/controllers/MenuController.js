...
const addMenu = async (req, res, next) => {
  try {
    ...
    bodyData.isApproved
      ...
      : 0;
    bodyData.menuType
      ? (bodyData["menuType"] = parseInt(bodyData.menuType))
      : 0;
    // update edited time
...
const updateMenu = async (req, res, next) => {
  try {
    ...
    const isValidate = await MenuSchema.checkUpdateInputValidate(
      bodyData,
      restaurentId
    ...
    if (req.files && req.files.menuBanner && req.files.menuBanner[0].filename) {
      ...
      if (oldMenuData && oldMenuData.menuBanner)
        //delete old file
        await helper.removeMenuFile(
          oldMenuData.menuBanner,
          "MENU",
          restaurentId
        );
    }
...
const deleteMenu = async (req, res, next) => {
  try {
    ...
    await model.Menu.findOneAndRemove({ _id: menuId, restaurentId }).exec(
