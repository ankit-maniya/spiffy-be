...
const addItem = async (req, res, next) => {
    try {
        ...
        bodyData.itemType ? bodyData["itemType"] = parseInt(bodyData.itemType) : 0
        ...
        if (req.files && req.files.itemBanner) {
            ...
            await helper.moveMenuFile(restaurentId.toString(), itemData._id, "ITEM", itemData.itemBanner);
        }
...
const updateItem = async (req, res, next) => {
    try {
        ...
        const isValidate = await ItemSchema.checkUpdateInputValidate(bodyData, restaurentId); // validate a key and value
        if (isValidate.statuscode != 1) {
            if (
                req.files &&
                req.files.itemBanner &&
        ...
        if (req.files && req.files.itemBanner && req.files.itemBanner[0].filename) {
            ...
            if (oldMenuData && oldMenuData.itemBanner) //delete old file
                await helper.removeItemFile(oldMenuData.itemBanner, "ITEM", restaurentId);
        }
...
const deleteItem = async (req, res, next) => {
    try {
        ...
        // extrect the menuid
        const { itemId } = req.params;
        await model.Item.findOneAndRemove({ _id: itemId, restaurentId }).exec(async (delError, delResponse) => {
            if (delError) {
                ...
                if (delResponse) {
                    ...
                    if (delResponse.itemBanner) {
                        debugger
                        await helper.removeItemFile(delResponse.itemBanner, "ITEM", delResponse.restaurentId);
                    }
