...
const logIn = async (req, res, next) => {
    try {
        ...
        const iData = await model.Restaurent.findOne({ //find Restaurent with this mobile 
            mobile: data.mobile,
        });
...
const signUp = async (req, res, next) => {
    try {
        ...
        if (validation) {
            ...
            if (req.files && req.files.restaurentBanner && req.files.restaurentBanner[0].filename) { // if error remove file 
        ...
        if (req.files && req.files.restaurentBanner && req.files.restaurentBanner[0].filename) { // set restaurent banner for add name in database
        ...
        if (req.files && req.files.profile) { // move file from TEMP location to RESTAURENT
            await moveFile(RestaurentData.profile, RestaurentData._id, "RESTAURENT");
        }
        if (req.files && req.files.restaurentBanner) { // move file from TEMP location to RESTAURENT
            await moveFile(RestaurentData.restaurentBanner, RestaurentData._id, "RESTAURENT");
        }
...
const updateRestaurent = async (req, res, next) => {
    try {
        ...
        const { _id, profile } = req.Restaurent;  // login Restaurent data
        const updateData = JSON.parse(JSON.stringify(req.body)); // remove unusual [obj]
        const validation = await UpdateRestaurentValidate(updateData); // validate a key and value
        if (validation) {
            if (req.files && req.files.profile && req.files.profile[0].filename) { // if error remove file 
                removeFile(req.files.profile[0].filename, "TEMP");
            }
        ...
        if (req.files && req.files.restaurentBanner && req.files.restaurentBanner[0].filename) { // set Restaurent Banner for add name in database
            ...
            if (restaurentBanner) { //delete old file
                removeFile(restaurentBanner, "RESTAURENT", _id);
            }
        }
        const Restaurent = await model.Restaurent.findByIdAndUpdate( // update Restaurent data and get latest data
            {
                _id: _id,
            },
            {
